...
if [ -f ${video} ]; then
  log "Created video ${video} from files in ${input_dir}, marking source images for purge."
  echo "touch ${img_dir}/.purge"
else
  log "Video ${video} does not exist, keeping source images."
fi

log "run_encode.sh completed successfully."
