#!/bin/bash

#
# Source some utility functions.
#
DIR="${BASH_SOURCE%/*}"
if [[ ! -d "$DIR" ]]; then
  DIR="$PWD"
fi

#
# base directory.
#
camera_host=raspi2.fritz.box
camera_user=pi
img_base_dir=/home/pi/images/raspi2/incoming
img_arch_dir=${img_base_dir}

log_file=$HOME/svc/logs/archive.log

. "$DIR/log.inc"
. "$DIR/parse.inc"

#
# Uses the imgdir computed by the parse_image_file_name function
# in parse.inc to copy the image file to correct daily directory.
function copy_image_file() {
  img=$1

  log "Image=${img}, image time=${imgtime}, image directory=${imgdir}"

  if [[ -z ${imgdir} ]]; then
    log "No image dir for ${img}, skipping."
    return 3
  fi

  src_path=${img_base_dir}/${img}
  dest_dir=${img_arch_dir}/${imgdir}
  dest_img=${img_arch_dir}/${imgdir}/${img}

  # Verify the source image does indeed exist.
  if [ ! -r ${src_path} ]; then
    log "Image ${src_path} doesn not exist or is not readable, skipping..."
    return 1
  fi

  # Create destination directory if necessary.
  if [ ! -d ${dest_dir} ]; then
    log "Destination directory ${dest_dir} does not exist, creating it... "
    # mkdir -p ${dest_dir}
    echo "done."
  fi

  echo "Copying ${src_path} to ${dest_img}"
  # cp -p ${src_path} ${dest_dir}
  ret=$?

  if [ $ret -ne 0 ]; then
    echo "Could not copy ${src_path} to ${dest_dir} ($ret)".
    return 2
  fi

  if [ -r ${dest_img} ]; then
    log "Verified file ${dest_img} exists, deleting source image ${src_path}"
    # rm ${src_path}
  else
    log "${dest_img} does not exist!"
  fi
}

function usage() {
  echo "Usage: `basename $0` file-list"
}


#
# Main part.
#
if [ $# -lt 1 ]; then
  usage
  exit 1
fi

if [[ $# -eq 2 ]]; then
  prefix=$2
else
  prefix=r_
fi

# Go to image directory to ensure wildcard is expanded.
cd ${img_base_dir}
if [ $? -ne 0 ]; then
  log "Could not change directory to ${img_base_dir}, exiting."
  exit 3
fi

files=$*
for i in ${files}; do
  # echo ${i}
  parse_image_file_name ${i} ${prefix}
  copy_image_file ${i}
done
